...
| # | Module / Page | Test Action | Expected Result | Tables Affected | Op | Classification | PASS/FAIL | Notes |
|---|--------------|-------------|-----------------|-----------------|-----|---------------|-----------|-------|
| 2.1 | Network Scopes | Load the page | Project tabs shown: Zain, Nokia, Huawei, IPT, TAC, MRC (in sort order); MOJ/General absent | `projects`, `sections` | R | **CRITICAL** | **PASS** | Patch 08 verified. Confirmed 2026-08-15. |
| 2.2 | Network Scopes | Select a project tab (e.g. Zain) | Sections listed (ftk, tdd, addsector); migrated rows present | `sections`, `rows` | R | **CRITICAL** | **PASS** | Confirmed 2026-08-15. Zain/FTK section subsequently retired as an intentional business decision (see 2.5). |
| 2.3 | Network Scopes | Add a new section to an existing project | Section appears in list; `section_name`, `section_label`, `project_name`, `columns`, `is_deleted`, `is_custom` all set | `sections` | I | **CRITICAL** | **PASS** | Confirmed 2026-08-15. Activity log: "Added Section MRC Project/FTK" at 12:54 and "Added Section MRC Project/TDD" at 14:10. |
| 2.4 | Network Scopes | Edit a section's label or columns | Updated values appear immediately; correct row updated | `sections` | U | **CRITICAL** | **PASS** | Confirmed 2026-08-15. Activity log: Added column "bb" → deleted column "bb" on mrc/ftk. 5 custom columns also added and persisted (Delivery, Installation, etc). |
| 2.5 | Network Scopes | Soft-delete (retire) a section | Section no longer appears in active list; `is_deleted = true` in DB | `sections` | U | **CRITICAL** | **PASS** | Confirmed 2026-08-15. Feature worked correctly. zain/ftk (e8ee675d, 467 rows) was intentionally retired as a business decision on this date — `is_deleted=true` is the correct final state. Rows retained in DB for historical/reference purposes. `docs/go-live/11_fix_remap_target_section_visibility.sql` superseded and must NOT be applied. |
| 2.6 | Network Scopes | Add a row to a section | Row appears in section table; `data`, `row_order`, `section_id` set correctly | `rows` | I | **CRITICAL** | **PASS** | Confirmed 2026-08-15 by user. Activity log: "Added Row MRC Project/FTK". |
| 2.7 | Network Scopes | Edit a row's cell value | Change persists on reload; `rows.data` JSONB updated | `rows` | U | **CRITICAL** | **PASS** | Confirmed 2026-08-15 by user. Activity log: "Edited Row MRC Project/FTK". |
| 2.8 | Network Scopes | Delete a row | Row removed; `rows` count decreases | `rows` | D | **CRITICAL** | **PASS** | Confirmed 2026-08-15 by user. Activity log: multiple "Deleted Row" entries confirmed. |
| 2.9 | Network Scopes | Reorder rows (drag or reorder action) | New order persists; `row_order` values updated | `rows` | U | IMPORTANT | | Not explicitly tested in 2026-08-15 session. |
| 2.10 | Network Scopes | Import rows via Excel/CSV upload | Rows inserted for correct section; schema unchanged | `rows` | I | IMPORTANT | **PASS** | Fixed 2026-08-15: title-row detection in `handleImportFile` (skips JSR brand title row); schema protection in `doImportWith` (headers must match section columns exactly, no `sections.columns` update); safe INSERT-first-then-DELETE pattern (no data loss on partial failure); always-show preview dialog. 23/23 unit parse tests pass, 40/40 round-trip QA tests pass. QA section `3813b019` restored. Activity log: "Imported Data MRC Project/FTK" confirmed. |
| 2.11 | Network Scopes | Export section to Excel | XLSX downloaded; contains correct column headers and all rows | `rows` | R | IMPORTANT | **PASS** | Confirmed 2026-08-15 by user. |
