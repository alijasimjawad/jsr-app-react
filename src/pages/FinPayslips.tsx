...
import { getTotalAdvanceDeductionForMonth, type Advance, type AdvanceDistribution } from '../lib/advances';
...
function printPayslip(row: PayslipRow, month: number, year: number) {
  ...
  const deductionRows = [
    ...(row.isAdjusted && row.adjType === 'deduction' ? [
      `<tr><td style="color:#dc2626">Deduction${row.adjReason ? ' — ' + e(row.adjReason) : ''}</td>
       <td style="text-align:right;color:#dc2626;font-weight:700">− ${fmt(row.adjAmount)}</td></tr>`,
    ] : []),
    ...(row.advanceDeduction > 0 ? [
      `<tr><td style="color:#dc2626">Salary Advance</td>
       <td style="text-align:right;color:#dc2626;font-weight:700">− ${fmt(row.advanceDeduction)}</td></tr>`,
    ] : []),
  ].join('');
        ...
        {row.advanceDeduction > 0 && (
          <>
            ...
            <div className={css.netPayItem}>
              ...
              <div className={css.netPayValue} style={{ color: '#f87171' }}>−{iqd(row.advanceDeduction)}</div>
            </div>
          </>
        )}
        <div className={css.netPayDivider} />
...
export default function FinPayslips() {
  ...
  const loadAll = useCallback(async (m: number, y: number) => {
    ...
    const [teamRes, adjRes, claimRes, advRes, advDistRes] = await Promise.all([
      ...
      // Salary advances (سلفة) — fetched org-wide (not month-filtered here)
      // so getTotalAdvanceDeductionForMonth can compute both the recipient
      // deduction and, for team leaders, the undistributed-remainder
      // deduction for this exact month/year. See src/lib/advances.ts.
      supabase.from('advances').select('*'),
      supabase.from('advance_distributions').select('*'),
    ]);
    ...
    setAdvDists((advDistRes.data as AdvanceDistribution[]) || []);
    setLoading(false);
  }, []);
  ...
  const rows: PayslipRow[] = teamWithSalary.map(m => {
    ...
    // Advance deduction = what this person personally received this month
    // (as a recipient of anyone's batch) PLUS, if they're a team leader,
    // whatever they still haven't distributed from their own batch(es)
    // targeting this month — computed live, no manual "close batch" step.
        ...
        <>
          ...
          <div className={css.kpiRow}>
            ...
            <div className={css.kpiCard}>
              ...
              <div className={`${css.kpiValue} ${css.kpiRed}`}>{iqd(totalAdvances)}</div>
              <div className={css.kpiSub}>salary advances this month</div>
            </div>
            <div className={css.kpiCard}>
              ...
              <table className={css.table}>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className={css.num} style={{ color: '#dc2626' }}>{totalAdvances > 0 ? `−${iqd(totalAdvances)}` : iqd(0)}</td>
